...
本项目的 **GitHub 仓库默认是公开的**，方便你继续维护和接收反馈。仓库公开主要意味着源码可以被任何人阅读，但不会直接暴露客户 NAS 上的运行实例（真实密钥都在客户自己的 `.env` 里，不在仓库中）。

为了降低被扫描/分析的风险，建议：

1. **把 GitHub Container Registry 上的镜像设为 Private**。
2. 给客户的 GitHub 账号单独授权读取这两个镜像。
3. 客户在极空间 NAS 上登录一次 Docker 后即可拉取私有镜像。

如果你按上面的方式操作，部署向导里选择“私有镜像”，会自动生成 `docker-login-极空间.txt` 登录说明。
